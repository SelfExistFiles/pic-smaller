...
async function loadPageComponent(pathname: string) {
  try {
    ...
    // 对于需要通用布局的页面，使用 CommonLayout 包装
    const pagesWithLayout = ['about', 'faq', 'compare', 'contact', 'privacy', 'guide', 'formats', 'usecases', 'technology', 'online-image-compression', 'bulk-image-processing', 'supported-formats'];
    
    if (pagesWithLayout.includes(pathname)) {
      const CommonLayout = (await import('@/components/CommonLayout')).default;
      return (
        <CommonLayout>
          <result.default />
        </CommonLayout>

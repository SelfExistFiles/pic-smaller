...
function getCurentLangStr(): string | undefined {
  const findLang = langList.find((item) => item && 'key' in item && item.key == gstate.lang) as { key: string; label: string } | undefined;
  return findLang?.label;
}

const CommonHeader = observer(() => {
    ...
    >
      <Space size="large">
        <div onClick={() => goto('/')} style={{ cursor: 'pointer' }}>
          <Logo title="Pic Smaller" />
        </div>
        <Typography.Link onClick={() => goto('/')} style={{ display: 'flex', alignItems: 'center' }}>
      ...
      <Space>
        ...
        >
          <Flex className="locale" align="center" style={{ 
            cursor: 'pointer',
            minWidth: '120px',
            whiteSpace: 'nowrap'
          }}>
            <svg viewBox="0 0 1024 1024" style={{ color: "currentcolor", width: '20px', height: '20px', marginRight: '6px' }}>
            ...
            <Typography.Text style={{ whiteSpace: 'nowrap' }}>{getCurentLangStr()}</Typography.Text>
          </Flex>
        </Dropdown>
        <Typography.Link
          target="_blank"
...
const CommonLayout = observer(({ children }: CommonLayoutProps) => {
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <CommonHeader />
      <Layout.Content style={{ padding: '20px', marginTop: '64px' }}>
        {children}
      </Layout.Content>
      <Footer />
    </Layout>
